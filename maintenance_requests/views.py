...
def list_requests(request):
	requests = maintenance_requests.objects.all()
...
def add_requests(request):
	...
	if serializer.is_valid():
		requestsSerializer.objects.create(
...
def check_requests(pk):
	try:
		room_type = maintenance_requests.objects.get(pk = pk)
	except maintenance_requests.DoesNotExist:
...
def update_requests(request, pk):
	...
	if serializer.is_valid():
		requests.guest_id = serializer.validated_data['guest_id']
